...
try:
    from apex import amp  # ty:ignore[unresolved-import]
...
class FastAttention(nn.Module):
    ...
    def forward(self, q, k, v, output_attentions=False):
        ...
        if output_attentions:
            ...
            v_diag = (
                v_diag.unsqueeze(0).unsqueeze(0).repeat(v.shape[0], v.shape[1], 1, 1)
            )
            ...
            attn_weights = (
                torch.zeros(1, 1, q.shape[2], q.shape[2]).to(device).to(torch.float16)
            )
...
class Performer(nn.Module):
    def __init__(
                    ...
                    [
                        ...
                        wrapper_fn(
                            Chunk(
                                ff_chunks,
                                FeedForward(
                                    dim, mult=ff_mult, dropout=ff_dropout, glu=ff_glu
                                ),
                                along_dim=1,
                            )
                        ),
        ...
        context_route_map = (
            {"context": route_context, "context_mask": route_context}
            if cross_attend
            else {}
        )
...
class PerformerLM(nn.Module):
    ...
    def forward(self, x, return_encodings=False, output_attentions=False, **kwargs):
        ...
        if output_attentions:
            x, attn_weights = self.performer(
                x, pos_emb=layer_pos_emb, output_attentions=output_attentions, **kwargs
            )
            # norm and to logits
            x = self.norm(x)
            if return_encodings:
