...
parser.add_argument(
    ...
    help="Path of data for finetune.",
)
parser.add_argument(
    "--model_path",
    type=str,
    default="./panglao_pretrained.pth",
    help="Path of pretrained model.",
)
parser.add_argument(
    "--ckpt_dir", type=str, default="./ckpts/", help="Directory of checkpoint to save."
)
parser.add_argument(
    "--model_name", type=str, default="finetune", help="Finetuned model name."
)
...
label_dict, label = np.unique(
    np.array(data.obs["celltype"]), return_inverse=True
    ...
    if i % VALIDATE_EVERY == 0:
        ...
        with torch.no_grad():
            for index, (data_v, labels_v) in enumerate(val_loader):
                ...
                final[
                    np.amax(np.array(final_prob.cpu()), axis=-1) < UNASSIGN_THRES
                ] = -1
            ...
            predictions = distributed_concat(
                torch.cat(predictions, dim=0), len(val_sampler.dataset), world_size
            )
            truths = distributed_concat(
                torch.cat(truths, dim=0), len(val_sampler.dataset), world_size
            )
            ...
            if is_master:
                print(
                    f"    ==  Epoch: {i} | Validation Loss: {val_loss:.6f} | F1 Score: {f1:.6f}  =="
                )
                print(confusion_matrix(truths, predictions))
                print(
                    classification_report(
                        truths, predictions, target_names=label_dict.tolist(), digits=4
                    )
                )
            if cur_acc > max_acc:
                max_acc = cur_acc
                trigger_times = 0
                save_best_ckpt(
                    i, model, optimizer, scheduler, val_loss, model_name, ckpt_dir
                )
            else:
                trigger_times += 1
